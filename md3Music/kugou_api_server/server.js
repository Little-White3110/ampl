...
async function registerDeviceAndGetDfid() {
  try {
    ...
    if (response.data?.data?.dfid) {
      ...
      console.log(`✅ 设备注册成功: dfid=${registeredDfid}${registeredMid ? ', mid=' + registeredMid : ''}`);
      
      deviceConfig.setDfid(registeredDfid);
      if (registeredMid) {
        deviceConfig.setMid(registeredMid);
      }
      ...
      fs.writeFileSync(
        path.join(__dirname, 'device_info.json'),
        JSON.stringify(deviceInfo, null, 2)
      );
      
      return deviceInfo;
    } else {
      console.warn('⚠️ 设备注册响应无效');
    }
  } catch (e) {
    console.warn('⚠️ 设备注册失败:', e.message);
  }
  return null;
}
...
function loadCachedDeviceInfo() {
  try {
    const cacheFile = path.join(__dirname, 'device_info.json');
    if (fs.existsSync(cacheFile)) {
      ...
      if (data.dfid) {
        ...
        deviceConfig.initDeviceInfo();
        console.log(`📁 加载缓存设备信息: dfid=${registeredDfid}, mid=${deviceConfig.getMid()}`);
        return true;
      }
    }
  } catch (e) {
    console.warn('⚠️ 加载设备缓存失败');
  }
  return false;
}

function getRegisteredDfid() {
  return registeredDfid;
}

function getRegisteredMid() {
...
async function startService() {
  ...
  appExt.service = app.listen(port, host, async () => {
    console.log(`✅ server running @ http://${host || 'localhost'}:${port}`);

    console.log('🔄 初始化设备信息...');
    if (!loadCachedDeviceInfo()) {
      console.log('📝 未找到缓存，开始注册设备...');
      await registerDeviceAndGetDfid();
    }
  });
...
module.exports = { startService, getModulesDefinitions, getRegisteredDfid, getRegisteredMid };
