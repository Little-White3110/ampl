// server_entry.js - Entry point for esbuild bundling
// This file pre-requires all modules so they're bundled inline

// Pre-require all modules
const modules = {};
const moduleNames = [
  ...
  'longaudio_album_audios','longaudio_album_detail','longaudio_daily_recommend',
  'longaudio_rank_recommend','longaudio_vip_recommend','longaudio_week_recommend',
...
for (const name of moduleNames) {
  try {
    modules[name] = require(`./module/${name}.js`);
  } catch (e) {
    // Skip
  }
}

// Build module definitions for server
...
// Override the module loading in server.js by passing moduleDefs
...
async function startLocalService() {
  ...
  // We need to rebuild the server with our moduleDefs
  // The simplest way: call startService which uses consturctServer
  // But we need to pass moduleDefs. Let's modify the approach:

  // Store moduleDefs globally so server.js can pick them up
  global.__BUNDLED_MODULE_DEFS__ = moduleDefs;

  // Call the original startService
  await startService();
}
